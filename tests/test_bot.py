...
class TestHomework:
    ...
    RETRY_PERIOD = 600
    INVALID_RESPONSES = {
        "no_homework_key": utils.InvalidResponse({"current_date": 123246}, "homeworks"),
    ...
    def test_bot_init_not_global(self, homework_module):
        for var in homework_module.__dict__:
            assert not isinstance(getattr(homework_module, var), telegram.Bot), (
    ...
    def test_logger(self, homework_module):
        ...
        hw_source = utils.get_clean_source_code(inspect.getsource(homework_module))
    ...
    def test_bot_initialized_in_main(self, homework_module):
        ...
        bot_init_with_token_pattern = re.compile(r"Bot\( *token *= *TELEGRAM_TOKEN *\)")
